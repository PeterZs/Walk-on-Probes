...
void
HCSolver<ScalarType, DIM>::configure(const nlohmann::json& j)
{
...
template<typename ScalarType, int DIM>
void
HCSolver<ScalarType, DIM>::runIteration(int iter)
{
...
#pragma omp parallel for schedule(dynamic)
        for (int p = 0; p < numProbes; ++p) {
            size_t start = probeBoundaryOffsets_[p];
            size_t end = probeBoundaryOffsets_[p + 1];
            if (start == end)
                continue;

            auto& probe = probesVec[p];
            for (size_t k = start; k < end; ++k) {
                probe.addSampleUnlocked(allBoundaryPoints_[k], allBoundaryResults_[k], pdf);
            }
        }
    }
...
template<typename ScalarType, int DIM>
void
HCSolver<ScalarType, DIM>::sampleBoundarySamples(const Probe<ScalarType, DIM>& probe,
                                                 int N,
...
template<typename ScalarType, int DIM>
void
HCSolver<ScalarType, DIM>::buildCache()
{
    const auto& scene = this->scene();

    if (targetPoints_.empty()) {
        spdlog::warn("HCSolver::buildCache: no target points set, skipping");
        return;
    }
    ...
    if (probes_.empty()) {
        spdlog::warn("HCSolver::buildCache: no probes placed, skipping");
        return;
    }

    spdlog::info("HCSolver: {} probes placed for {} target points", probes_.probes().size(), targetPoints_.size());

    // 2. Initialize boundary sample data structures and Fourier structures
    probeBoundaryCounts_.clear();
    probeBoundaryOffsets_.clear();
    probeBoundaryCounts_.reserve(probes_.probes().size());
    probeBoundaryOffsets_.reserve(probes_.probes().size() + 1);
    probeBoundaryOffsets_.push_back(0);
    std::vector<Vector<DIM>> boundarySamples;
    for (auto& probe : probes_.probes()) {
        int N = std::max(static_cast<int>(lambda_ * std::pow(probe.radius, DIM - 1)), minBoundarySamples_);
        probeBoundaryCounts_.push_back(N);
        probeBoundaryOffsets_.push_back(probeBoundaryOffsets_.back() + N);
    ...
    cacheBuilt_ = true;
    spdlog::info("HCSolver: cache built successfully");
}
...
template<typename ScalarType, int DIM>
ScalarType
HCSolver<ScalarType, DIM>::solvePoint(const Vector<DIM>& p,
                                      int pointIdx,
                                      pcg64& rng,
...
template<typename ScalarType, int DIM>
void
HCSolver<ScalarType, DIM>::solve(const std::vector<Vector<DIM>>& points, std::vector<ScalarType>& results)
{
    if (!cacheBuilt_) {
...
template class WOS_SOLVER_API HCSolver<Scalar<1>, 2>;
template class WOS_SOLVER_API HCSolver<Scalar<1>, 3>;
template class WOS_SOLVER_API HCSolver<Scalar<3>, 2>;
template class WOS_SOLVER_API HCSolver<Scalar<3>, 3>;
